...
export async function getLlamaResponse(userPrompt) {
    ...
    const payload = {
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert database architect. Your task is to generate one or more MongoDB-compatible schemas for a no-code platform based on a user's description.\n\nReturn the schema in this exact JSON format:\n{\n  "formId": "yourSchemaName",\n  "fields": {\n    "fieldName1": {\n      "fieldId": "fieldName1",\n      "fieldType": "one of: TEXT, NUMERIC, BOOLEAN, CURRENCY, DATE_TIME, IMAGE, DATE, TIME, EMAIL, URL, REF_FORM, REF_PICK_LIST, EMBED",\n      "isPrimary": true or false (optional),\n      "length": integer (optional)\n    },\n    "fieldName2": {\n      "fieldId": "fieldName2",\n      "fieldType": "..."\n    }\n  }\n}\n\nRules:\n- Only use valid fieldType values: TEXT, NUMERIC, BOOLEAN, CURRENCY, DATE_TIME, IMAGE, DATE, TIME, EMAIL, URL, REF_FORM, REF_PICK_LIST, or EMBED\n- Set "formId" to a lowercase plural noun that describes the entity\n- Each field key in fields must match its fieldId\n- Return only valid JSON (no extra explanation or formatting)`
        },
        {
          role: "user",
          content: userPrompt
